...
impl AppState {
    ...
    fn orchestrate(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
    ...
    fn scan_context(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
    ...
    fn generate_plan(&self, Parameters(p): Parameters<ConfirmParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
    ...
    fn generate_prd(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
    ...
    fn architect_backend(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
        ...
        Json(format!("Architecture saved: {}\n\n{}", path.display(), arch))
    }
    ...
    fn design_schema(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
    ...
    fn componentize_ui(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
        ...
        Json(format!("Component tree saved: {}\n\n{}", path.display(), comp))
    }
    ...
    fn dispatch_build(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
        match dispatch::execute(project_root, &p.task, "plan approved", &Default::default()) {
            Ok(()) => Json("✅ Dispatched. Agent is building...".to_string()),
            Err(e) => Json(format!("❌ Dispatch failed: {}", e)),
        }
    }
    ...
    fn run_tests(&self) -> Json<String> {
        ...
        for cmd in &commands {
                ...
                .output()
            {
                ...
                if stdout.contains("pass") || stdout.contains("ok") || out.status.success() {
                    return Json(format!("🧪 Tests:\n{}\n{}", stdout, stderr));
                }
            }
        }
        Json("🧪 No test runner detected. Run tests manually.".to_string())
    }
    ...
    fn record_session(&self, Parameters(p): Parameters<TaskParams>) -> Json<String> {
        let project_root = self.palbox.parent().unwrap_or(std::path::Path::new("."));
        ...
        Json(format!("✅ Session recorded: {}", path.display()))
    }
}
